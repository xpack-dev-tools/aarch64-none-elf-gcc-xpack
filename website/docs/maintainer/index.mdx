...
title: How to make new xPack GNU AArch64 Embedded GCC releases
...
```sh
xpm run clear-all-project-builds -C ~/Work/xpack-dev-tools/aarch64-none-elf-gcc-xpack.git/build-assets
```
...
npm dist-tag add @xpack-dev-tools/aarch64-none-elf-gcc@${customField('upstreamVersion')}-${customField('xpackSubversion')}.${customField('npmSubversion')} latest
npm dist-tag ls @xpack-dev-tools/aarch64-none-elf-gcc
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`npm unpublish @xpack-dev-tools/aarch64-none-elf-gcc@${customField('upstreamVersion')}-${customField('xpackSubversion')}.${customField('npmSubversion')}
`} </CodeBlock>
