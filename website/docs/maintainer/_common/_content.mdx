...
**@xpack-dev-tools/aarch64-none-elf-gcc** is an xpm package that packages platform-specific archives, built with the [xPack Build Box (XBB)](https://xpack.github.io/xpack-build-box/).
...
Execute the Docker build on the production machine (`berry5`);
start a VS Code remote session, or connect with a terminal:

```sh
caffeinate ssh berry5
```
...
(such as **15.2.1-1.1.1**) with the following expression:
...
SSH sessions to the build machines (`xbbli`, `xbbmi`, `xbbma`, `berry5`):
...
For `xbbli` & `berry5`, which have additional memory, start two runners:
...
- In `CHANGELOG.md`, add the release date and a message
  such as _**\* v15.2.1-1.1 released**_
...
tagged such as **v15.2.1-1.1** (mind the dash in the middle!) and
named such as **xPack GNU AArch64 Embedded GCC v15.2.1-1.1** (mind the dash),
...
- **Commit** all changes with the message
  _**package.json: update URLs for 15.2.1-1.1.1 release**_
...
- Navigate to the [GitHub Releases](https://github.com/xpack-dev-tools/aarch64-none-elf-gcc-xpack/releases) page
