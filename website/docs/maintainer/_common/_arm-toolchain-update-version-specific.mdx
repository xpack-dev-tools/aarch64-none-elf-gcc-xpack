...
- create a branch like <code>arm-{customField('gdbVersionMajor')}-aarch64-none-elf-{versionMajor()}.{versionMinor()}.rel{customField('armSubRelease')}</code>
...
the <code>arm-gnu-toolchain-aarch64-none-elf-abe-manifest.txt</code> file
