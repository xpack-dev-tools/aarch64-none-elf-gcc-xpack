...
it uses **the same format as npm**. xPacks can be installed from the
same repositories as npm packages, whether public or private. Additionally,
binary xPacks include standard archives with the binaries, such as
`.tar.gz` or `.zip` for Windows.


xPacks are managed by **xpm** (the xPack Project Manager)
which is built on top of **npm**, the
popular JavaScript package manager. It extends npm with new
